...
if (tsPlugin && tsParser) {
  configs.push({
    ...
    rules: {
      ...
      // TypeScript's own compiler is authoritative for undefined identifiers
      // (including ambient type namespaces like NodeJS.ErrnoException, which
      // base no-undef cannot see without full type information).
      'no-undef': 'off',
    },
  });
}
